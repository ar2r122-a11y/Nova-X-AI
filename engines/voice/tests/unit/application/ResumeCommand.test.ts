...
describe("ResumeCommand", () => {
    it("creates with required voiceId", () => {
        const command = new ResumeCommand("voice-1");
        expect(command.voiceId).toBe("voice-1");
    });

    it("generates correlationId by default", () => {
        const command = new ResumeCommand("voice-1");
        expect(command.correlationId).toMatch(/^resume-\d+$/);
    });

    it("uses custom correlationId when provided", () => {
        const command = new ResumeCommand("voice-1", "custom-corr");
    ...
    it("accepts custom claims", () => {
        ...
        const command = new ResumeCommand("voice-1", "corr", "caus", claims);
